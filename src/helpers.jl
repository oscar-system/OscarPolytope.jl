...
homogenize(mat::AbstractMatrix, val::Number = 1) =
    augment(mat, fill(val, size(mat, 1)))

dehomogenize(vec::AbstractVector) = vec[2:end]
dehomogenize(mat::AbstractMatrix, rowidcs = 1:size(mat, 1)) =
    mat[rowidcs, 2:end]

iscomputed(HP::HomogeneousPolyhedron, property::Symbol) =
    Polymake.exists(HP.polymakePolytope, string(property))
iscomputed(P::Polyhedron, property::Symbol) =
    iscomputed(P.homogeneous_polyhedron, property)
